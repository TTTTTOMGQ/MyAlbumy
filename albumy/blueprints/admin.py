...
from albumy.models import User, Role, Tag, Comment, Photo
...
def lock_user(user_id):
    ...
    if user.locked:
        flash('User already locked', 'warning')
    user.lock()
...
def block_user(user_id):
    ...
    if user.locked:
        flash('User already blocked', 'warning')
    user.block()
...
def manage_photo(order):
    ...
    order_rule = 'flag'
    if order == 'by_time':
        pagination = Photo.query.order_by(Photo.timestamp.desc()).paginate(page=page, per_page=per_page)
        order_rule = 'time'
    else:
...
def manage_comment(order):
    ...
    return render_template('admin/manage_comment.html',
                           pagination=pagination, comments=comments, order_rule=order_rule)
